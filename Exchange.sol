...
contract Exchange {
    ...
    function withdrawLiquidity(uint _liquidityPositionsToBurn) external returns (uint amountEthToSend, uint amountERC20ToSend) {
        require(_liquidityPositionsToBurn > 0, "Must burn more than 0 positions");
    ...
    function swapForEth(uint _amountERC20Token) external returns (uint ethToSend) {
        ...
        require(ethToSend > 0, "Insufficient output amount");
        require(ethToSend <= ethReserve, "Insufficient ETH liquidity in pool");
        require(ethToSend <= address(this).balance, "Insufficient ETH balance in contract");
    ...
    function swapForERC20Token() external payable returns (uint ERC20TokenToSend) {
        ...
        require(ERC20TokenToSend > 0, "Insufficient output amount");
        require(ERC20TokenToSend <= tokenReserve, "Insufficient ERC20 liquidity in pool");
        require(ERC20TokenToSend <= token.balanceOf(address(this)), "Insufficient ERC20 balance in contract");
